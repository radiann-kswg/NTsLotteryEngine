...
- **実機の落下方位は一様ではない**（120〜150° と 300° に山、0〜90° が谷）。MC の校正値は「方位一様の平均」なので、
  当たり扇形が 270° の塔は高く、0/180° の p32 は低い。**実測（塔だけ Play ×65）**:

  | variant（塔） | 実測 p（全段） | 95% CI | 目標＝校正値 |
  | --- | --- | --- | --- |
  | p10（1・0） | **0.169** | [0.117, 0.237] | 0.100 ✗ |
  | p6of88（10） | **0.123** | [0.064, 0.225] | 0.068 ✗ |
  | p18（81） | **0.261** | [0.181, 0.362] | 0.181 △ |
  | p32（64） | **0.226** | [0.150, 0.326] | 0.318 △ |
  | p50（2） | **0.539** | [0.457, 0.619] | 0.500 ✓ |
  | p53（33） | **0.536** | [0.453, 0.616] | 0.529 ✓ |

- **懸念だった「投入方位が固定だから当落が決まる」は実機では起きていなかった**（喉から落ちる球がばらけて Δ の中央値 105〜115°・分布が広い）。
  段ごと（最上段 135° / 奇数段 270° / 偶数段 90°）の差は概ね 95% CI 内。ただし p18 の奇数段だけ高い傾向（0.533・n=15）。
- 詳細な表・方位ヒスト・直しかたの候補は `docs/DESIGN.md` 2.2。**次はここを直すのが最優先**。
...
1. **当選率のズレを直す**（`docs/DESIGN.md` 2.2 (d)）。安い順:
   - (a) **当たり扇形を円周に散らす**（例 p10: 37°×1 → 9.25°×4 を 0/90/180/270 に）。`kuruun_params.json` の `variants.*.win` → `gen_kuruun.py` → MC 再校正 → **実機 Play ×65 で再測定**（`LotoPlayLoop.Start(65, "Output/verifyN")`。1 回 ≈ 24 秒）。
   - (b) 根治: コレクタの当たり／ハズレ円盤だけをボウルと違う回転数で回す（樋は静止のまま分離）。Blender の分割が要る。
   どちらを採るかは User 判断。**直したら (a)(b) いずれでも実機 Play で測り直すこと**（MC だけでは足りないことが今回わかった）。
