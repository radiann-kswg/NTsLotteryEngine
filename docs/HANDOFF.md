...
6. ボールテクスチャの実装（進行中・2026-09-18 時点 **6 / 106 球**: ロト 6・50・58・63・85・93）: `Assets/Textures/BallSkins/` に `BallTex_NTS-{Num_Badge}.png` を置いたら `Tools > NTsLoto > Build Ball View Scene`（空き行へ自動割当・冪等）→ `Tools > NTsLoto > Capture Ball Skins`（BallViewScene を開いた状態で。プレビュー撮影と README 収録表の書き戻し）。Inspector での手貼りは個別上書きのときだけ。規約は同フォルダの `README.md`、運用は `AGENTS.md` 4 章 7 と 8 章。
